...
public class MenuManager : MonoBehaviour {
	...
	private float previousOrthoSizeMenu = 1f;
	private float cameraMenuScalar = 1f;

	// Use this for initialization
	void Start () {
		currentOrthoSize = Camera.main.orthographicSize;
		previousOrthoSize = currentOrthoSize;
		previousOrthoSizeMenu = currentOrthoSize;
	...
	void newMenu( GameObject tile ) // makes new menu facing camera, sets menuMad  to true
	{
		...
		if( currentOrthoSize != previousOrthoSizeMenu )
		{
			cameraMenuScalar *= currentOrthoSize/previousOrthoSizeMenu;
			//Debug.Log( cameraMenuScalar + " == " + currentOrthoSize + "/" + previousOrthoSizeMenu );

			previousOrthoSizeMenu = currentOrthoSize;
		}

		menu.transform.localScale = Camera.main.transform.localScale;
		menu.transform.localScale *= cameraMenuScalar * ConstantsLib.ICON_MENU_SIZE;

		/*
		currentOrthoSize = Camera.main.orthographicSize;
		
		if( menu != null && currentOrthoSize != previousOrthoSize )
		{
			cameraScalar = currentOrthoSize/previousOrthoSize;
			menu.transform.localScale *= ( cameraScalar );			
			previousOrthoSize = currentOrthoSize;
		}
		 * */
	...
	{	
		...
		float finalColliderRad = 1f; //g.GetComponent<SphereCollider>().radius;		

		if (g.gameObject.tag == ConstantsLib.WBC_TAG)
		{
			finalColliderRad = ConstantsLib.WBC_PROJ_RANGE 
				/ g.gameObject.transform.localScale.magnitude;
		}
		else if(  g.gameObject.tag == ConstantsLib.MUCUS_TAG )
		{
			finalColliderRad = ConstantsLib.MUCUS_PROJ_RANGE
				/ g.gameObject.transform.localScale.magnitude;			
		}
		else if(  g.gameObject.tag == ConstantsLib.MORTAR_TAG )
		{
			finalColliderRad = ConstantsLib.MORTAR_PROJ_RANGE
				/ g.gameObject.transform.localScale.magnitude;
		}
		else if(  g.gameObject.tag == ConstantsLib.SYRINGE_TAG )
		{
			finalColliderRad = ConstantsLib.SYRINGE_PROJ_RANGE
				/ g.gameObject.transform.localScale.magnitude;
		}
		else if(  g.gameObject.tag == ConstantsLib.TESLA_TAG )
		{
			finalColliderRad = ConstantsLib.TESLA_PROJ_RANGE
				/ g.gameObject.transform.localScale.magnitude;
		}
	...
	void towerPreview( GameObject g, Vector3 hoverPoint )
	{
		hoverPoint.y += ConstantsLib.Y_OFFSET;
		//Debug.Log ("previed");
		previewTower = (GameObject)Instantiate (g, hoverPoint, Quaternion.Euler( 0, 90, 0 ) );
	...
	void towerWhenYouCLick( GameObject g, GameObject plane, Vector3 hoverPoint ) 
	{
		...
		wbc = (GameObject)Instantiate (g, hoverPoint, randomAngle() );
